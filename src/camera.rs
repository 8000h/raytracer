...
use crate::geometry::{ Ray, Hittable, HittableGroup, Interval };
...
impl Camera {
	pub fn new(position: Vec3f, lookat: Vec3f, fov: f32, image_width: u32, image_height: u32) -> Camera {
		...
		let pixel_dx = vx / image_width as f32;
		let pixel_dy = vy / image_height as f32;

		let viewport_corner = (position - cz) - vx / 2.0 - vy / 2.0;
		let pixel_corner = viewport_corner + pixel_dx / 2.0 + pixel_dy / 2.0;

		Camera { position, pixel_dx, pixel_dy, pixel_corner }
	}
	...
	pub fn intial_ray(&self, pixel_x: u32, pixel_y: u32) -> Ray {
		let point = self.pixel_corner
			+ (self.pixel_dx * pixel_x as f32)
			+ (self.pixel_dy * pixel_y as f32);
	...
	pub fn raycast(&self, ray: &Ray, world: &HittableGroup, depth: u32) -> Vec3f {
		...
		if let Some(hit_result) = world.hit(&Interval::new_universe(), ray) {
			if let Some((attenuation, scattered)) = hit_result.material.scatter(ray, &hit_result) {
				return attenuation * self.raycast(&scattered, world, depth - 1);
			}
